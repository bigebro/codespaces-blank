...
export function exportAuditExcel(
  ...
  const pnlData: any[][] = [
    ...
    ["1. Нийт борлуулалтын орлого (ПОС, НӨАТ-тай дүн)", `${Math.round(fin.revenue || 0).toLocaleString()} ₮`, "-"],
    ["2. Тооцоолсон НӨАТ (10%)", `${Math.round(tax.estimated_vat_10pct || 0).toLocaleString()} ₮`, "-"],
    [`3. ${tax.tax_mode || 'Хялбаршуулсан ААНОАТ'}`, `${Math.round(tax.active_tax_amount || 0).toLocaleString()} ₮`, tax.is_above_300m ? "10.0%" : "1.0%"],
    [],
    ["ДАНС БА ЗАРДЛЫН НЭР", "ДҮН (₮)", "ОРЛОГОД ЭЗЛЭХ %"],
    ["I. ҮНДСЭН ҮЙЛ АЖИЛЛАГААНЫ ЦЭВЭР ОРЛОГО (НӨАТ-гүй)", `${Math.round(fin.net_revenue || 0).toLocaleString()} ₮`, "100.0%"],
    [],
  ...
  const cashflowData: any[][] = [
    ...
    ["• Үндсэн хөрөнгө худалдан авалт (Тоног төхөөрөмж)", `0 ₮`, `-${(cf.fixed_assets_paid || 0).toLocaleString()} ₮`, `-${(cf.fixed_assets_paid || 0).toLocaleString()} ₮`],
