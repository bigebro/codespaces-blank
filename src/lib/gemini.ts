...
export function advancedMongolianVoiceParser(
  ...
  const numMatch = text.match(
    /(\d+(?:\.\d+)?)\s*(л|литр|l|мл|ml|кг|kg|гр|грамм|gram|ш|ширхэг|хайрцаг|уут|багц|сав)?/,
  );
  ...
  if (/асг|мууд|гаш|хая|цуц|хагар|уна|дуус|эвд|алд|урс|түлс/.test(text))
    type = "spoilage";
  else if (/ава|авс|татан|ирл|нэм|худалд|оруул|авчир/.test(text))
    type = "purchase";
...
export function fastLocalParse(text: string, ingredientsList: string[]) {
  ...
  const numMatch = lower.match(
    /(\d+(?:\.\d+)?)\s*(л|литр|l|мл|ml|кг|kg|гр|грамм|gram|ш|ширхэг)?/,
  );
  if (!numMatch) return null;
  ...
  const matchedIng = (ingredientsList || []).find((ing) =>
    lower.includes(ing.toLowerCase().trim()),
  );
...
export async function parseOperationalText(
  ...
  const localResult = advancedMongolianVoiceParser(
    text,
    ingredientsList,
    learnedAliases,
  );
...
const key = (process.env.GEMINI_API_KEY || "").replace(/["']/g, "").trim();
  ...
  if (key) {
    try {
      ...
      console.error("parseOperationalText Error:", lastError);
    }
  } else {
    lastError = "GEMINI_API_KEY олдсонгүй (.env.local шалгана уу)";
  }
  ...
  try {
    ...
    return {
      ...
      success: false,
      error_message: "JSON уншиж чадсангүй.",
    };
  }
}

// Add this helper function in src/lib/gemini.ts:
...
async function parseReceiptWithGroqVision(
  ...
  if (!res.ok || !data.choices?.[0]?.message?.content) {
    throw new Error(data.error?.message || `Groq Vision error (HTTP ${res.status})`);
  }
...
// Replace your parseReceiptImage function in src/lib/gemini.ts with this:
...
const systemPrompt = `
  You are an expert receipt OCR assistant. Read the provided Mongolian receipt image carefully.
  Extract every purchased line item into the "purchases" JSON array with ONLY these 4 keys:
  - "item_name": (string, exact product name printed on receipt)
  - "quantity": (number, quantity bought)
  - "total_cost": (number, total price paid for this line)
  - "is_food": (boolean, true if edible restaurant food/ingredient, false if cleaning/packaging/household supply)
  ...
  Respond STRICTLY with this JSON format:
  {
    ...
    "purchases": [
      {
        "item_name": "Мишээл ХАР ТАЛХ",
        "quantity": 2,
        "total_cost": 7800,
        "is_food": true
      },
      {
        "item_name": "Гарын саван",
        "quantity": 1,
        "total_cost": 2900,
        "is_food": false
      }
...
`;
  // Strip prefix if the base64 string contains "data:image/...;base64,"
                const cleanBase64 = base64Image.replace(/^data:image\/[a-z]+;base64,/, "");
                const geminiKey = (process.env.GEMINI_API_KEY || "").replace(/["']/g, "").trim();

                // ⚡ 1. Primary Attempt: Google Gemini 3.5 Flash-Lite
                if (geminiKey) {
                  try {
      ...
      console.warn("⚠️ Gemini receipt OCR failed, falling back to Groq Qwen 3.8 Vision:", e.message);
    }
  }

  // ⚡ 2. Fallback: Groq Qwen 3.8 27B Vision
  try {
    const groqResult = await parseReceiptWithGroqVision(cleanBase64, systemPrompt);
    return groqResult;
  } catch (groqErr: any) {
    console.error("❌ Both Gemini and Groq Vision failed:", groqErr.message);
    return {
      success: false,
      error_message: "Зургийг уншиж чадсангүй. Арай тод дарж дахин илгээнэ үү.",
    };
  }
}